...
extern void* create_con(int oper, void* leftcon, void* rightcon);
...
__inline Relation* con_execute(Relation*, DBnode* db, struct con* con);
...
Relation* con_execute(Relation* rela, DBnode* db, struct con* con) {
	int i = 1;
	switch (con->con_type)
	{
	case JOIN:
...
int parse_select(DBnode* db,Token** curr) {
	Vector v_col;
	Vector v_from;
	VECTOR_INIT(&v_col, 4);
	...
	rela_print(con_execute(NULL, db, con_));
	//泄漏 Vector* v_col *v_from;
