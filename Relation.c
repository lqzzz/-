...
Relation* rela_loop_join(Relation * left, Relation* right) {
	...
	Relation* join_rela = rela_create(data_len, 1, copy_left, VectorGetUsedSize(out_v) * VectorGetUsedSize(inter_v));
	...
	VECTOR_FOREACH(out_var, out_v,
		VECTOR_FOREACH(inter_var, inter_v,
			...
			memcpy(join_data, out_var->data_, left_data_len);
			memcpy((char*)join_data + left_data_len, inter_var->data_, right_data_len);
			obj* o = create_obj(join_data);
			inc_ref(o);
...
struct rehead* rela_search_col(Relation* rela, Pair* schema){
	...
	char* target_col_name = PairGetSecond(schema);
	LIST_FOREACH(node_, head_,
		...
		if (target_table_name == NULL) {
			if (strcmp(target_col_name, col_name) == 0)
				return node_;
		}else if (strcmp(target_col_name, col_name) == 0 &&
...
Relation* get_relation(Table* table){
	...
	int rec_count = table->used_rec_size;
	Vector* vec_rec = &table->rec_vec;
	Relation *rela_ = rela_create(table->rec_len, 0, headlist, rec_count);
	Vector* data_set = &rela_->data_set;
	for (int i = 0; i < rec_count; i++) {
		size_t data_len = table->rec_len;
		Record* rec_src = VECTOR_GET_VALUE(vec_rec, i);
		void* data_ = mem_alloc(data_len);
		memcpy(data_, rec_src->data_, data_len);
		obj* o = create_obj(data_);
		inc_ref(o);
		VECTOR_PUSHBACK(data_set, o);
	}
	return rela_;
}
...
__inline Relation* rela_filter_col_col(Relation* rela, Pair* first, Pair* second, int opertype) {
	...
	//}else {
	//	for (int i = 0; i < len_; i++) {
	//		char* data_ = rec_data[i];
	//		if (data_) {
	//			if (filter_cmp(data_ + offset_f, data_ + offset_s, opertype, data_type) == 0) {
	//				rec_data[i] = NULL;
	//			}
	//		}
	//	};
	//}
	//if (is_join) {
	//	VECTOR_FOREACH(value_, data_set,
	//		if (filter_cmp((char*)value_ + offset_f, (char*)value_ + offset_s, opertype, data_type))
	//			VECTOR_PUSHBACK(new_data_set, value_);
	//		else
	//			mem_free(value_);
	//	);
...
__inline Relation* rela_filter_col_key(Relation* rela, Pair* schema, void* target,int opertype) {
	...
	//		char *data_ = rec_data[i];
	//		if (data_) {
	//			if (filter_cmp(data_ + offset_, target, opertype, data_type) == 0) {
	//				rec_data[i] = NULL;
	//			}
	//		}
	//	};
	//}
	//struct rehead* col_ = rela_search_col(rela, schema);
	//Relation* new_rela = rela_create(rela->data_len, rela->is_join, rela->rehead_list, VectorGetUsedSize(&rela->data_set));
	//Vector* data_set = &rela->data_set;
	//int data_type = col_->data_type;
	//int offset_ = col_->rec_offset;
	//int is_join = rela->is_join;
	...
	//if (is_join) {
	//	VECTOR_FOREACH(value_, data_set,
	//		if (filter_cmp((char*)value_ + offset_, target, opertype, data_type))
	//			VECTOR_PUSHBACK(new_data_set, value_);
	//		else
	//			mem_free(value_);
	//	);
